...
function FavoriteButton({ drinkId, email }) {
  ...
  const fetchUserFavoriteDrinks = async () => {
    try {
      const response = await fetch("https://cocktail-creator-backend.vercel.app/user_favorite_drinks_button");
  ...
  const handleFavoriteClick = async () => {
    try {
      if (isDrinkFavorited) {
        ...
        const response = await fetch(`https://cocktail-creator-backend.vercel.app/user_favorite_drinks/${drinkId}`, {
          method: "DELETE",
        });
        if (response.ok) {
        ...
        const response = await fetch("https://cocktail-creator-backend.vercel.app/favorite_drinks", {
          method: "POST",
          headers: {
